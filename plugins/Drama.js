...
function dramaParse() {
  let drama = pick(SENTENCES);

  for (const [key, values] of Object.entries(WORDS)) {
    drama = drama.replace(new RegExp(`\\[${key}\\]`, 'g'), () => pick(values));
  }
...
module.exports = {
  ...
  commands: [{
    name: 'drama',
    help: 'Generates random drama using Mod Developers, Projects, and other semi random data.'
  }],

  init() {
    console.log('[Drama] initialized');
  },

  handleCommand(ctx) {
    const target = ctx.replyTarget || ctx.to;
    const users = channelState.getUsers(target);
    addressedSay(ctx, antiPingMessage(dramaParse(), users));
  },
