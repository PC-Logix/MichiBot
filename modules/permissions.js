...
const { resolveDiscordUserIdFromBridge } = require('../services/bridgeAuth');
...
function openDb() {
  ...
  migrateLegacyUserTableIfNeeded(db);
...
function migrateLegacyUserTableIfNeeded(database) {
  try {
...
function canAccess(ctx, access) {
  const effective = access || { public: true };
